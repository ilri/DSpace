...
public class ConfigurationRestRepositoryIT extends AbstractControllerIntegrationTest {
    ...
    public void getSingleValue() throws Exception {
        ...
        getClient(adminToken).perform(get("/api/config/properties/configuration.exposed.single.value"))
    ...
    public void getArrayValue() throws Exception {
        String adminToken = getAuthToken(admin.getEmail(), password);
        getClient(adminToken).perform(get("/api/config/properties/configuration.exposed.array.value"))
    ...
    public void getNonExistingValue() throws Exception {
        String adminToken = getAuthToken(admin.getEmail(), password);
        getClient(adminToken).perform(get("/api/config/properties/configuration.not.existing"))
    ...
    public void getNonExposedValue() throws Exception {
        String adminToken = getAuthToken(admin.getEmail(), password);
        getClient(adminToken).perform(get("/api/config/properties/configuration.not.exposed"))
            .andExpect(status().isNotFound());
    }
