...
<xsl:stylesheet
    ...
    <xsl:template name="itemSummaryView-DIM-affiliations">
        ...
        <xsl:if test="dim:field[@mdschema='cg' and @element='contributor' and @qualifier='initiative']">
            <div class="simple-item-view-description item-page-field-wrapper table">
                <h5 class="bold"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-initiative</i18n:text></h5>
                <div>
                    <xsl:for-each select="dim:field[@mdschema='cg' and @element='contributor' and @qualifier='initiative']">
                        <xsl:choose>
                            <xsl:when test="node()">
                                <xsl:call-template name="discovery-link">
                                    <xsl:with-param name="filtertype" select="'initiative'"/>
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:text>&#160;</xsl:text>
                        ...
                        <xsl:if test="count(following-sibling::dim:field[@mdschema='cg' and @element='contributor' and @qualifier='initiative']) != 0">
                            <xsl:text>; </xsl:text>
                        </xsl:if>
                    </xsl:for-each>
                </div>
    ...
    <xsl:template name="itemSummaryView-DIM-author-affiliation">
        <xsl:if test="dim:field[@mdschema='cg' and @element='contributor' and @qualifier='affiliation']">
            <div class="word-break item-page-field-wrapper table">
                <h5 class="bold">
                    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-author-affiliation</i18n:text>
                </h5>
                <xsl:for-each select="dim:field[@mdschema='cg' and @element='contributor' and @qualifier='affiliation' ]">
                    <a target="_blank" >
                        <xsl:attribute name="href">
                            <xsl:value-of select="concat($context-path,'/discover?filtertype=affiliation&amp;filter_relational_operator=equals&amp;filter=',url:encode(node()))"></xsl:value-of>
                        </xsl:attribute>
                        <xsl:copy-of select="./node()"/>
                    </a>
