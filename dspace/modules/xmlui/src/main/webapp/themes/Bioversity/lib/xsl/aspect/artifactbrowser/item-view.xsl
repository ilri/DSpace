...
<xsl:stylesheet
	...
	<!-- CRP Subject row -->
		...
		<xsl:when test="$clause = 22 and (dim:field[@element='subject' and @qualifier='bioversity'])">
	                ...
	                <td>
	                <xsl:for-each select="//dim:field[@element='bioversity'and @qualifier='bioversity']">
                      <xsl:variable name="lnk"><xsl:value-of select="."/></xsl:variable>

                            <a href="/browse?value={.}&amp;type=bioversityt"><xsl:value-of select="."/></a><xsl:if test="position()!=last()"><xsl:text>, </xsl:text></xsl:if>
